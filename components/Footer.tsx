...
const Footer: React.FC<FooterProps> = ({ isLoggedIn, onLoginClick, onLogoutClick, theme, toggleTheme }) => {
      ...
      <div className="container mx-auto">
        ...
        <div className="border-t border-slate-200 dark:border-slate-700/50 mt-8 pt-6 flex flex-col sm:flex-row justify-between items-center gap-6 text-center sm:text-left">
           ...
           <div className="flex items-center gap-4 flex-shrink-0">
              <button
                  onClick={toggleTheme}
                  className="p-3 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors"
                  aria-label="Cambiar tema"
              >
                  {theme === 'light' ? ICONS.moon : ICONS.sun}
              </button>
              {isLoggedIn ? (
                <button onClick={onLogoutClick} className="text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors text-xs">Cerrar Sesión</button>
              ) : (

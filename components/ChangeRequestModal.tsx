...
const ChangeRequestModal: React.FC<ChangeRequestModalProps> = ({ instruction, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
      <div className="bg-slate-800 rounded-lg shadow-2xl w-full max-w-lg">
        <div className="flex justify-between items-center p-4 border-b border-slate-700">
          <h2 className="text-xl font-display text-amber-300">{getTitle()} - Listo para Publicar</h2>
        ...
        <div className="p-6">
          ...
          <pre className="bg-slate-900 text-slate-300 p-4 rounded-md text-xs overflow-x-auto max-h-60">
            <code>
              {instructionText}
            </code>
          </pre>
        ...
        <div className="flex justify-between items-center p-4 border-t border-slate-700 bg-slate-800/50">
           ...
           <div>
            <button type="button" onClick={onClose} className="bg-slate-600 text-white font-bold py-2 px-4 rounded-md hover:bg-slate-500 mr-2">
                Cerrar
            </button>
             <button
                type="button"
                onClick={handleCopy}
                className="bg-amber-400 text-slate-900 font-bold py-2 px-4 rounded-md hover:bg-amber-300 flex items-center gap-2"
              >
                {ICONS.copy}
                {copyButtonText}
            </button>

    ...
    >
      ...
      >
        <div className="flex justify-between items-center p-4 border-b border-slate-700">
          <h2 className="text-2xl font-display text-amber-300">Información sobre {town}</h2>
          <button type="button" onClick={onClose} className="text-slate-400 hover:text-white">{ICONS.close}</button>
        </div>
        
        <div className="p-6 text-slate-300 leading-relaxed overflow-y-auto">
          <FormattedText text={content} />
        </div>
        
        <div className="flex justify-end items-center gap-4 p-4 border-t border-slate-700 bg-slate-800/50 mt-auto">
          <button type="button" onClick={onClose} className="bg-amber-400 text-slate-900 font-bold py-2 px-6 rounded-md hover:bg-amber-300 transition-colors">
            Cerrar
          </button>
        </div>
      </div>

    ...
    <>
      <div className="animate-fade-in max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
            <button onClick={onBack} className="flex items-center gap-2 text-amber-300 hover:text-amber-200 font-bold transition-colors">
            ...
            {isLoggedIn && (
                <button 
                    onClick={onEdit}
                    className="flex items-center gap-2 bg-slate-600 text-white font-bold py-2 px-4 rounded-md hover:bg-slate-500 transition-colors"
                >
        ...
        <article className="bg-slate-800 rounded-lg shadow-lg overflow-hidden">
          {imageUrl && (
            <div className="w-full h-48 sm:h-64 bg-black/20">
                <img 
                  src={imageUrl} 
                  alt={`Imagen de ${title}`} 
          ...
          <div className={`p-6 sm:p-8 border-t-4 ${colors.border}`}>
              ...
              <div className="flex gap-2 flex-wrap">
                  {interestInfo && (
                       <button
                         ...
                         className="flex items-center justify-center gap-2 bg-slate-700 text-slate-300 font-bold py-2 px-4 rounded-md hover:bg-slate-600 transition-colors text-base"
                       >
                         {ICONS.map}
            ...
            <div className="text-slate-300 text-base sm:text-lg leading-relaxed mb-8">
              <FormattedText text={description} />
            </div>

            <div className="pt-6 border-t border-slate-700/50">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-lg font-bold text-slate-300">Comparte este evento</h3>
                <button 
              ...
              <div className="flex items-center gap-4">
                <a href={twitterUrl} target="_blank" rel="noopener noreferrer" title="Compartir en X/Twitter" className="p-3 rounded-full bg-slate-700 text-slate-300 hover:bg-slate-600 hover:text-white transition-colors">
                  {ICONS.twitter}
                </a>
                <a href={facebookUrl} target="_blank" rel="noopener noreferrer" title="Compartir en Facebook" className="p-3 rounded-full bg-slate-700 text-slate-300 hover:bg-slate-600 hover:text-white transition-colors">
                  {ICONS.facebook}
                </a>
                <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" title="Compartir en WhatsApp" className="p-3 rounded-full bg-slate-700 text-slate-300 hover:bg-slate-600 hover:text-white transition-colors">
                  {ICONS.whatsapp}
                </a>
                <a href={emailUrl} title="Compartir por Email" className="p-3 rounded-full bg-slate-700 text-slate-300 hover:bg-slate-600 hover:text-white transition-colors">
                  {ICONS.email}
                </a>
              </div>
            </div>
